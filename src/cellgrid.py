...
class CellGrid_CartesianShell():
    ...
    def cell_in_bounds(self, i, j, k):
        ...
        print(self.cell_center(i,j,k))
        print(self.cell_center(i,j,k).norm())
        print('in block', in_block)
        # print('in hash', in_hash)
        print('in_sparse', in_sparse)
    ...
    def project_ray_to_bounds_square(self, start, dir):
        ...
        print("start", start)
        print("dir", dir)
        print("r:", self.r0, self.r1)
        print("Projections:", *((t, (start + t * dir)) for t in (*t0p, *t1p, *t0m, *t1m)), sep='\n\t')
    ...
    def compute_ray_cells(self, start, end, include_ray_edges = True):
        ...
        if len(order) > 0:
            ...
            print(cells, indices, cells[indices.nonzero()])
